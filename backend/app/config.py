...
Environment Variables (see .env):
    WEALTHNEST_DATA_DIR: Override production data directory (default: ./backend/data/prod)
    WEALTHNEST_TEST_MODE: When "1", use test data directory (backend/data/test/)
    ...
    FRONTEND_BASE_URL: Public frontend URL used in emailed links (default: http://localhost:8000)
"""
...
def set_test_mode(enabled: bool = True):
    ...
    """
    os.environ["WEALTHNEST_TEST_MODE"] = "1" if enabled else "0"
...
class Settings(BaseSettings):
    ...
    # Portfolio
    PORTFOLIO_BASE_CURRENCY: str = "INR"
    ...
    model_config = ConfigDict(
        ...
        extra="ignore",  # Ignore env vars not defined as fields (e.g. WEALTHNEST_DATA_DIR)
    )
...
def get_data_dir() -> Path:
    ...
    2. WEALTHNEST_DATA_DIR env var → custom path
    3. Default → backend/data/prod/
    """
    if is_test_mode():
        return DEFAULT_TEST_DATA_DIR

    env_data_dir = os.environ.get("WEALTHNEST_DATA_DIR")
    if env_data_dir:
...
def get_settings() -> Settings:
    ...
    LOG_LEVEL can be overridden by WEALTHNEST_LOG_LEVEL env var.
    """
    settings = Settings()
    settings.DATABASE_URL = get_database_url()

    log_level_override = os.environ.get("WEALTHNEST_LOG_LEVEL")
