...
2.  **Storage & Analysis**:
    -   The backend saves the file with a unique **UUID** to prevent filename conflicts and path traversal attacks. The original filename is stored in a `.json` metadata sidecar file.
