...
### Plans APPENA CREATI (in `RoadmapV4_UI/`)
| File | Descrizione | Priorità |
|------|-------------|----------|
...
## 🗺️ Ordine Esecuzione Prossimi Step

### Step 1: i18n CLI Improvements (30 min) 📋
**Riferimento**: `plan-i18n-cli-improvements.md`
...
**Riferimento**: `plan-files-ux-refactor.md` (Step 2)
**Obiettivo**: Permettere deep-linking ai filtri della pagina `/files`
- URL params: `?tab=static|brim&broker_id=X&status=Y&search=Z`
- Lettura params da URL all'apertura pagina
- Aggiornamento URL quando si cambiano filtri (senza reload)
- Link da broker detail → `/files?tab=brim&broker_id=X`

### Step 3: Files UX Refactoring (2h) 📋
**Riferimento**: `plan-files-ux-refactor.md` (Step 3)
**Obiettivo**: Migliorare UX sezione import files in broker detail
- Convertire sezione in modale (`BrokerImportFilesModal.svelte`)
- Usare DataTable standard (non lista custom)
- Delete con ConfirmModal (non `confirm()`)
- Upload auto-assegna broker senza chiederlo
